    ...
    139c:	89 d1                	mov    %edx,%ecx
{
  ...
  do{
    ...
    13c9:	0f b6 92 30 18 00 00 	movzbl 0x1830(%edx),%edx
    ...
    1449:	8b 5d 0c             	mov    0xc(%ebp),%ebx
{
  ...
  for(i = 0; fmt[i]; i++){
    ...
    14b9:	ff 24 85 d8 17 00 00 	jmp    *0x17d8(,%eax,4)
        putc(fd, c);
      }
      state = 0;
    }
  ...
  write(fd, &c, 1);
    ...
    15c7:	90                   	nop
          s = "(null)";
    15c8:	ba d1 17 00 00       	mov    $0x17d1,%edx
        while(*s != 0){
...
free(void *ap)
{
  ...
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
    15e1:	a1 28 1b 00 00       	mov    0x1b28,%eax
{
  ...
  freep = p;
    ...
    1621:	5b                   	pop    %ebx
  freep = p;
    1622:	89 15 28 1b 00 00    	mov    %edx,0x1b28
}
    1628:	5e                   	pop    %esi
    1629:	5f                   	pop    %edi
    162a:	5d                   	pop    %ebp
  ...
  freep = p;
    165c:	89 15 28 1b 00 00    	mov    %edx,0x1b28
    p->s.size += bp->s.size;
...
malloc(uint nbytes)
{
  ...
  if((prevp = freep) == 0){
    167c:	8b 3d 28 1b 00 00    	mov    0x1b28,%edi
  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
  ...
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    ...
    if(p == freep)
    16c1:	8b 3d 28 1b 00 00    	mov    0x1b28,%edi
  ...
  return freep;
    16ef:	8b 15 28 1b 00 00    	mov    0x1b28,%edx
    ...
    171c:	89 70 04             	mov    %esi,0x4(%eax)
      freep = prevp;
    171f:	89 15 28 1b 00 00    	mov    %edx,0x1b28
}
    ...
    172f:	c3                   	ret    
    base.s.ptr = freep = prevp = &base;
    1730:	c7 05 28 1b 00 00 2c 	movl   $0x1b2c,0x1b28
    1737:	1b 00 00 
    base.s.size = 0;
    173a:	bf 2c 1b 00 00       	mov    $0x1b2c,%edi
    base.s.ptr = freep = prevp = &base;
    173f:	c7 05 2c 1b 00 00 2c 	movl   $0x1b2c,0x1b2c
    1746:	1b 00 00 
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    1749:	89 fa                	mov    %edi,%edx
    base.s.size = 0;
    174b:	c7 05 30 1b 00 00 00 	movl   $0x0,0x1b30
    1752:	00 00 00 
    if(p->s.size >= nunits){
